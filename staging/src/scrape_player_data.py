...
import os
import sqlite3
from constant_values import get_general_stat_ids, get_type_dict, id_2_col, get_qb_sql_cols, get_passing_stat_ids, \
    create_passing_table, get_rushing_stat_ids, get_rushing_sql_cols, create_rushing_table, get_receiving_stat_ids, \
    get_receiving_sql_cols, create_receiving_table
import time
import json
...
def get_player_name(doc):
    ...
    for i in range(len(h1_elems)):
        ...
        for item in h1_elem.items():
            if item[1] == 'name':
                return str(h1_elems[i].text_content()).strip()
    return None
...
def add_passing_row(game_dict, add_player):
    ...
    if 'pass_att' in game_dict and game_dict['pass_att']:
        ...
        statement = '''INSERT INTO passing_statistics (%s) VALUES (%s)''' % (
        ','.join([ID2COL[key] for key in key_list]), ','.join(['?'] * len(key_list)))
...
def add_rushing_row(game_dict, add_player, increment_game_id):
    ...
    if 'rush_att' in game_dict and game_dict['rush_att']:
        ...
        key_list = list(game_dict.keys())
        values = [game_dict[key] for key in key_list]
        statement = '''INSERT INTO rushing_statistics (%s) VALUES (%s)''' % (
        ','.join([ID2COL[key] for key in key_list]), ','.join(['?'] * len(key_list)))
        new_cursor = conn.cursor()
        new_cursor.execute(statement, values)
    return added_player, increment_next
...
def add_receiving_row(game_dict, add_player, increment_game_id):
    ...
    if 'targets' in game_dict and game_dict['targets']:
        ...
        key_list = list(game_dict.keys())
        values = [game_dict[key] for key in key_list]
        statement = '''INSERT INTO receiving_statistics (%s) VALUES (%s)''' % (
        ','.join([ID2COL[key] for key in key_list]), ','.join(['?'] * len(key_list)))
        new_cursor = conn.cursor()
        new_cursor.execute(statement, values)
    return added_player
...
def scrapePlayerGameLogs(url):
    ...
    for i in range(len(tr_elements)):
        ...
        if isStatRow(tr_elem.items()):
            passing_game_dict = {'game_id': None, 'name': player_name, 'player_id': None,
                                 'playoffs': isPlayoffRow(tr_elem.items()), 'position': player_postion}
            rushing_game_dict = {'game_id': None, 'name': player_name, 'player_id': None,
                                 'playoffs': isPlayoffRow(tr_elem.items()), 'position': player_postion}
            receiving_game_dict = {'game_id': None, 'name': player_name, 'player_id': None,
                                   'playoffs': isPlayoffRow(tr_elem.items()), 'position': player_postion}
            for c in tr_elem:
